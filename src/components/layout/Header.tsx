...
const Header = ({ className, collapseBreakpointClass = 'lg' }: IHeader) => {
  ...
  const collapseBreakpointCss =
    collapseBreakPointCssMap[collapseBreakpointClass as keyof typeof collapseBreakPointCssMap]
  ...
  useEffect(() => {
    if (inputRef.current?.value) inputRef.current.value = String(searchParams?.s ?? '')
  }, [inputRef.current])

  return (
    <header className={clsx(className || 'header-light', HeaderStyle)}>
      <nav className={clsx(DesktopNavLinks, collapseBreakpointCss.desktopNavLinks)}>
        {DefaultLogoLink}
        <div>
          <input
            className={Input}
      ...
      <nav className={clsx(MobileNavLinksContainer, collapseBreakpointCss.mobileNavLinksContainer)}>
        {DefaultLogoLink}
        <MotionDiv
